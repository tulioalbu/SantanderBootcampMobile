...
// pois quase todos esses répteis são bem lentos. Cada tartaruga é classificada em
...
// A primeira linha contém um inteiro L (1 ≤ L ≤ 500) representando o número de
// tartarugas do grupo, e a segunda linha contém L inteiros Vi (1 ≤ Vi ≤ 50)
...
fun main(args: Array<String>) {
    ...
    for(i in 1..3) {
        val tamanho = readLine()!!.toInt()
        val velocidade = readLine()!!.split(" ").run { map { it.toInt() } }
        val maior = velocidade.toTypedArray()

        maior.sort()

        if(maior.last() < 10) println("1")
        else if (maior.last() < 20) println("2")
        else println("3")
    }
}
